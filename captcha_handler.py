...
class CaptchaResolver:
    ...
    def _predict_captcha_batch(self, mfcc_batch_tensor: Tensor) -> list[str]:
        ...
        for idx in predicted_indices:
            if idx < len(self.classification):
                ...
                print(f"ERROR: Predicted index {idx} out of bounds for classification list.")
    ...
    def _process_audio_to_text(self, audio_stream: io.BufferedIOBase) -> str:
        ...
        if not self._is_ffmpeg_available():
            print(f"ERROR: ffmpeg not found at path: {self.ffmpeg_path}. Cannot process audio.")
            return ""
        ...
        try:
            if hasattr(audio_stream, 'read'):
                ...
                print("ERROR: Audio stream is not readable or does not have a read method.")
                return ""
        ...
        try:
            ...
            process_result = subprocess.run(ffmpeg_cmd, input=input_bytes, capture_output=True, check=False)
            ...
            if process_result.returncode == 0:
                ...
                print(f"ERROR: ffmpeg processing failed. Return code: {process_result.returncode}")
                stderr_output = process_result.stderr.decode('utf-8', errors='ignore') if process_result.stderr else "No stderr output"
                print(f"ERROR: ffmpeg stderr: {stderr_output}")
                # y remains None
        except FileNotFoundError:
            print(f"ERROR: ffmpeg executable not found at '{self.ffmpeg_path}'. Please ensure ffmpeg is installed and in PATH or path is configured correctly.")
            return ""
        except Exception as e_ffmpeg:
            print(f"ERROR: Error during ffmpeg processing or librosa loading: {e_ffmpeg}")
            # y remains None
        
        if y is None or sr == 0:
            print("ERROR: Failed to load audio data using ffmpeg from the provided stream.")
            return ""

        loaded_duration_sec = len(y) / sr

        if loaded_duration_sec < MIN_REQUIRED_DURATION_SEC:
            print(f"ERROR: Audio duration ({loaded_duration_sec:.2f}s) is insufficient. Required: {MIN_REQUIRED_DURATION_SEC:.2f}s.")
            return ""
        ...
        try:
            for i in range(num_chars):
                ...
                if start_sample >= end_sample:
                    print(f"ERROR: Segment {i+1} has invalid time range after conversion: start_sample={start_sample}, end_sample={end_sample}")
                    print(f"ERROR: Error: Segment {i+1} results in empty or invalid sample range.")
                    return ""
            ...
            if len(mfcc_list) != num_chars:
                print(f"ERROR: Expected {num_chars} MFCC segments, but got {len(mfcc_list)}")
                return ""
    ...
    async def resolve_audio_captcha(self, session: aiohttp.ClientSession, audio_source: str) -> Coroutine[Any, Any, str]:
        """
        ...
        if audio_source.startswith(('http://', 'https://')):
            ...
            print(f"ERROR: Invalid audio source: Not a valid URL or existing file path: '{audio_source}'")
            return ""

        if audio_content is None:
            print(f"ERROR: Failed to obtain audio content from source: '{audio_source}'")
            return ""
