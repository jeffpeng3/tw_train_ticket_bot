...
class CaptchaResolver:
    ...
    def _wav2mfcc(
        ...
        n_fft_to_use = 2048
        if (
            len(wave) < n_fft_to_use
        ):  # This check might still be relevant for very short valid audio
    ...
    def _process_audio_to_text(
        self, audio_stream: io.BufferedIOBase, trans=True
    ) -> str:
        ...
        for startTime_ms in range(0, int(audio_duration_ms) + 1, step_ms):
            ...
            if mfcc is not None:
                all_mfccs_list.append(mfcc)  # Assuming mfcc is never None
                num_segments += 1
        ...
        for idx, _interval_len_for_calc in enumerate(intervals):
            ...
            all_calculated_core_indices.append(
                (_start_core_idx_calc, _end_core_idx_calc)
            )
        ...
        for i, interval_len_loop in enumerate(intervals):
            ...
            if start_core_idx >= L_raw:
                print(
                    f"Warning: Char {i + 1}: Start index {start_core_idx} for MFCC segment is out of bounds (L_raw={L_raw}). Appending '?'"
                )
            ...
            if start_core_idx >= actual_end_core_idx:
                print(
                    f"Warning: Char {i + 1}: Core MFCC segment is invalid or empty. Indices for all_mfccs_list: [{start_core_idx}:{actual_end_core_idx}], L_raw={L_raw}. Appending '?'"
                )
                predicted_char = "?"
            else:
                ...
                if not core_segment_mfccs:
                    print(
                        f"Warning: Char {i + 1}: Extracted core_segment_mfccs is empty for indices [{start_core_idx}:{actual_end_core_idx}]. Appending '?'"
                    )
                    predicted_char = "?"
                else:
                    try:
                        stacked_core_mfccs = np.stack(core_segment_mfccs)
                        mfcc_batch_tensor_for_segment = (
                            torch.from_numpy(stacked_core_mfccs).float().unsqueeze(1)
                        )

                        raw_chars_for_segment: str = self._predict_captcha_batch(
                            mfcc_batch_tensor_for_segment
                        )

                        predicted_char = self._get_char_from_raw_prediction_segment(
                            raw_chars_for_segment
                        )
                        if not predicted_char:
                            print(
                                f"Warning: Char {i + 1}: _get_char_from_raw_prediction_segment returned empty for raw_chars '{raw_chars_for_segment}'. Appending '?'"
                            )
                            predicted_char = "?"
                    except Exception as e_pred:
                        print(
                            f"ERROR: Char {i + 1}: Failed during prediction for MFCC segment [{start_core_idx}:{actual_end_core_idx}]. Error: {e_pred}. Appending '?'"
                        )
                        predicted_char = "?"
